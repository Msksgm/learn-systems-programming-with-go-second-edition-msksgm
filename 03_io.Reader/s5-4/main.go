...
func dumpCheck(chunk io.Reader) {
	var length int32
...
func main() {
	...
	dumpedFile, err := os.Open("PNG_transparency_demonstration_secret.png")
	if err != nil {
		panic(err)
	}

	// チャンクの書き込まれたコードの呼び出し
	dumpedChunks := readChunks(dumpedFile)
	for _, c := range dumpedChunks {
		dumpCheck(c)
	}
}
